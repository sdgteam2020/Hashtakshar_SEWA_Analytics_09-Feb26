...
public class ClientKeyHasher : IClientKeyHasher
{
    private const int SaltSize = 16;        // 128-bit
    private const int KeySize = 32;         // 256-bit
    private const int Iterations = 120_000; // good baseline

    public (string hashBase64, string saltBase64) Hash(string clientKey)
    {
        if (string.IsNullOrWhiteSpace(clientKey))
            throw new ArgumentException("ClientKey is empty.");

        byte[] salt = RandomNumberGenerator.GetBytes(SaltSize);

        byte[] hash = Rfc2898DeriveBytes.Pbkdf2(
            password: Encoding.UTF8.GetBytes(clientKey),
            salt: salt,
    ...
    public bool Verify(string clientKey, string hashBase64, string saltBase64)
    {
